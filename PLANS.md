...
Stages 10, 10A, 10B, 10C, and 10D are complete. Mock remains the offline server/test default, while the upload page recommends real local ASR/rule planning and selects its media Provider per task. Corrected transcript text is now the sole input to subtitles and semantic planning, and review renders rebuild renderer-derived media state. External-source mode is explicitly a non-commercial effect-validation prototype. Stage 11 is optional and remains deferred while the core chain stabilizes.
