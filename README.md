...
P4 connects those APIs to the existing page. Standard tasks retain their original four-stage progress display. Agent tasks hide estimated percentages and render the eight real node states from SSE, including resume/failure state. An approval panel displays structured reasons and the candidate plan, then calls the existing approve/edit/reject endpoints. Completed Agent tasks add a privacy-safe Trace tab with tool/model identifiers, retries, durations, and structured violation codes. Refreshing or restoring a task reloads authoritative task, approval, trace, and terminal node history from the backend.
