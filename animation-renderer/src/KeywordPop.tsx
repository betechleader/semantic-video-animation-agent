...
export const KeywordPop: React.FC<KeywordPopProps> = (props) => {
  ...
  return <div style={{position: 'absolute', ...positions[props.position], opacity, transform: `${positions[props.position].transform ?? ''} scale(${scale})`}}>
    <div style={{backgroundColor: props.color, borderRadius: 24, boxShadow: '0 12px 28px rgba(0,0,0,0.35)', color: '#111', fontFamily: 'Microsoft YaHei, sans-serif', fontSize: 72, fontWeight: 800, padding: '22px 34px', whiteSpace: 'nowrap'}}>
      {props.text}
    </div>
  </div>;
};
